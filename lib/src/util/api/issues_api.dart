...
class IssueApiClient {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        decodedResponse["results"].forEach((element) {
          issueList!.add(Issue.fromJson(element as Map<String,dynamic>));
        });
        issueData = IssueData(
          count: decodedResponse["count"] as int,
          nextQuery: decodedResponse["next"] as String?,
          previousQuery: decodedResponse["previous"] as String?,
          issueList: issueList,
        );
      }
    } catch (e) {
  ...
  static Future<Issue?> getIssueById(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var decodedResponse = jsonDecode(utf8.decode(response.bodyBytes));
        issue = Issue.fromJson(decodedResponse as Map<String,dynamic>);
      }
    } catch (e) {
      print(e);
    }
  ...
  static Future<void> postIssue(Issue issue, BuildContext parentContext) async {
    ...
    try {
      ...
      request.fields["url"] = issueMap["url"] as String;
      request.fields["status"] = issueMap["status"] as String;
      request.fields["description"] = issueMap["description"] as String;
  ...
  static Future<int?> getIssueLikes(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var decodedResponse = jsonDecode(utf8.decode(response.bodyBytes));
        likeCount = decodedResponse["likes"] as int?;
      }
    } catch (e) {
      print(e);
    }
  ...
  static Future<int?> getIssueFlag(int id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var decodedResponse = jsonDecode(utf8.decode(response.bodyBytes));
        flagCount = decodedResponse["flags"] as int?;
      }
    } catch (e) {
      print("e");
    }
